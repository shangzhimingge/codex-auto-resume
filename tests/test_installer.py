...
class InstallerTests(unittest.TestCase):
    ...
    def test_installer_is_idempotent_preserves_existing_agents_and_can_disable(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual("1.1.0", (installed / "VERSION").read_text(encoding="utf-8").strip())
    ...
    def test_runtime_and_distribution_versions_are_1_1_0(self):
        skill = ROOT / "skill" / "codex-auto-resume"
        self.assertEqual("1.1.0", (skill / "VERSION").read_text(encoding="utf-8").strip())
        namespace = {}
        exec((skill / "scripts" / "auto_resume" / "__init__.py").read_text(encoding="utf-8"), namespace)
        self.assertEqual("1.1.0", namespace["__version__"])
